...
class MyTestCase(unittest.TestCase):
    ...
    def test_something(self):
        result = self.G.get_incoming_transitions_for_list_of_states([self.R3])
        self.assertTrue(len(result) == 2)
        self.assertTrue(self.T_R2_R3 in result)
        self.assertTrue(self.T_R1_R3 in result)
    def test_something_2(self):
        result = self.G.get_incoming_transitions_for_list_of_states([self.R2])
        self.assertTrue(len(result) == 2)
        self.assertTrue(self.T_R1_R2 in result)
        self.assertTrue(self.T_R2_R2 in result)
    def test_something_3(self):
        result = self.G.get_incoming_transitions_for_list_of_states([self.R2, self.R3])
