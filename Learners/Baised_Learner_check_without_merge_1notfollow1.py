...
class Baised_Learner1:
    ...
    def run_EDSM_with_pattern_learner(self):
        ...
        for blue in self.blue_states:
            for red in self.red_states:
                ...
                if self.is_valid_merge(ds): # and not violate_hard_pattern(pattern_list, self.pta.G):
                    ...
                    violate_hard_pattern, violated_hard_label = is_violate_pattern_1NotFollowing1(ds, self.hard_patterns, self.pta.G)
        ...
        if valid_for_at_least_one_red:
            ds_with_highest_scour = self.pick_high_score_pair(mergable_states)
            write_to_file_in_new_line(self.with_pattern_file_name,f'{ds_with_highest_scour.merging_score}')
    ...
    def stop(self):
        ...
        if self.is_all_states_red():
            stop = True
        elif len(self.pta.G.get_all_states()) <= (self.sizeof_actual_system/2):
            stop = True
        ...
        for blue in self.blue_states:
            for red in self.red_states:
                ...
                if self.is_valid_merge(ds):  # and not violate_hard_pattern(pattern_list, self.pta.G):
                    ...
                    # write_to_file_in_new_line(self.with_pattern_file_name,f'Merging {ds.s1} & {ds.s2} => score: {ds.merging_score}' )
                    violate_hard_pattern, violated_hard_label = is_violate_pattern_1NotFollowing1(ds,
                                                                                                  self.hard_patterns,
                                                                                                  self.pta.G)
                    if violate_hard_pattern:
                        ds.merging_score = -1
                        ...
                        write_to_file_in_new_line(self.with_pattern_file_name,
                                                  f'Hard_Pattern_violation {ds.s1} & {ds.s2} => {violated_hard_label}')

                    if ds.merging_score > 0:
                        mergable_states.append(ds)
                        valid_for_at_least_one_red = True
                write_to_file_in_new_line(self.with_pattern_file_name,f'Try {ds.s1} & {ds.s2} = {ds.merging_score}' )
