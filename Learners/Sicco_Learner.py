...
class Sicco_Learner:
    ...
    def setup(self, tracker_file_name):
        ...
        self.APTA_size_after_merging_rejected_states = self.sizeof_actual_system

    def is_all_states_red(self):
        if len(self.red_states) != len(self.pta.G.graph.keys()):
            return False
        return True
    ...
    def stop(self):
        ...
        if self.is_all_states_red():
            stop = True
        elif len(self.pta.G.get_all_states()) <= (self.sizeof_actual_system/2):#self.sizeof_actual_system<=1000 and
            stop = True
        # elif self.sizeof_actual_system>1000 and len(self.pta.G.get_all_states())<=4000:
        #     stop = True
        return stop

    def run_Sicco_learner(self):
        #stop when all states are red or when the size of the learned model is half of the actual model
        if self.stop():
            return
        ...
        for blue in self.blue_states:
            for red in self.red_states:
                ...
                if self.is_valid_merge(ds):
                    add_new_transition = self.adding_new_transition(red, blue)
                    if not add_new_transition:
                ...
                write_to_file_in_new_line(self.tracker_file_name,
                                          f'Try {ds.s1} & {ds.s2} = {ds.merging_score}')
        ...
        if valid_for_at_least_one_red:
            ds_with_highest_score = self.pick_high_score_pair(mergable_states)
            ...
            if ds_with_highest_score.s1.get_reference_state() != ds_with_highest_score.s2.get_reference_state():
                ...
                write_to_file_in_new_line(self.tracker_file_name,
                                          f'Winner ==> {ds_with_highest_score.s1} & {ds_with_highest_score.s2} = {ds_with_highest_score.merging_score} - incorrect merge')
            else:
                self.correct_merge_counter += 1
                write_to_file_in_new_line(self.tracker_file_name,
                                          f'Winner ==> {ds_with_highest_score.s1} & {ds_with_highest_score.s2} = {ds_with_highest_score.merging_score} - correct merge')
            self.merge_sets(ds_with_highest_score)
            graph_to_dot(self.pta.G, f'merge_{self.figure_num}.dot')
            # dot_to_png(f'merge_{self.figure_num}.dot',f'merge_{self.figure_num}.png')
            # self.figure_num += 1

        self.update_red_states()
        self.run_Sicco_learner()
    ...
    def mergre_rejected_states(self):
        ...
        self.APTA_size_after_merging_rejected_states = len(self.pta.G.get_all_states())
        return target
    ...
    def adding_new_transition(self, red, blue):
        target_outgoing_transitions = self.pta.G.get_outgoing_transitions_for_state(red)
        ...
        source_outgoing_transitions = self.pta.G.get_outgoing_transitions_for_state(blue)
        for s_t in source_outgoing_transitions:
            if s_t.input_key not in target_transitions_list:
                return True
        return False
    ...
    def write_statistics(self, file_name):
        ...
        write_to_file_in_new_line(file_name, f'Size of APTA after merging rejected states: {self.APTA_size_after_merging_rejected_states}')
        ...
        write_to_file_in_new_line(file_name, f'Size of learned model(sicco_EDSM): {len(self.pta.G.get_all_states())}')
